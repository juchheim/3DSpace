...
Last updated: 2026-06-08 (wall-corner build piece)
...
- **2026-06-08:** **Wall/corner alignment + facing fixes.** (1) `wallCornerMeshTransform` rotation map was wrong — the GLB L natively hugs **south+east** (an `se` corner), bbox origin-centered; corrected `CORNER_ROTATION_Y` = `{se:0, sw:π/2, nw:π, ne:−π/2}` and position = cell center (verified each corner's render now matches its collider edges). (2) **Walls face the placer:** `wallFacingRotation()` stores a 0/180 flip on `rotation` from avatar vs edge-line side (GLB front = local +Z, confirmed: +Z half has ~1662 verts vs 458); `WallGlbMesh` adds the flip. Avatar X/Z threaded through `resolveBuildPlacementTarget` (3D `targetFromHit` + place-ahead). (3) `alignWallEdgeToNeighbors` now also snaps walls collinear to adjacent **wall-corner arms** (`CORNER_EDGES` map). NOTE: wall (~0.59 m deep) and corner arms (~0.40 m, inset) have different cross-sections — not pixel-perfect flush; user opted to keep the dedicated GLB. Two-wall-segment corner is the fallback for perfect flushness.
