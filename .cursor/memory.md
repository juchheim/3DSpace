...
- **FFA AI World Host:** `docs/planning/rooms/free-for-all/aibot/PLAN_FREE_FOR_ALL_AI_WORLD_HOST.md`, `IMPL_FREE_FOR_ALL_AI_WORLD_HOST.md` — named retro-robot NPC; build-help tutor + persistent file study chat; **Phase 1 contracts** on `feature/aibot`
...
- **2026-06-01:** **FFA AI World Host Phase 2 (API host CRUD)** — `ENABLE_AI_WORLD_HOST` config; Mongo `room_ai_hosts` + memory repo; `apps/api/src/ai-host/*`, `routes/ai-host.ts`; GET/POST/PATCH/DELETE host with realtime `room.ai-host.updated.v1` / `dismissed.v1`; display-name trim/charset/profanity; tests `ai-host-host.test.ts`. Phases 3–7 not started.
