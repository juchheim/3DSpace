...
- **2026-06-01:** **FFA AI World Host Phase 1 (contracts)** — `aiWorldHost` on `RoomTypeFeatureFlags` (true FFA only); `RoomSettings.aiWorldHost`; entities + `RoomAiHostRealtimeMessageSchema`; `fileId` required for `file-study`; `DismissRoomAiHostQuerySchema`; API error codes `ai-host-*`; OpenAPI regen; env flags; tests `ai-world-host.test.ts`. Phases 2–7 not started.
