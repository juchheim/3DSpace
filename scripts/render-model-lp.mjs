// Offline render of the MODEL-LP host GLB — proof/thumbnail helper that needs
// neither the dev server nor an env map. Serves the repo over a tiny static
// server, loads the GLB with three's GLTFLoader in headless Chromium, and
// screenshots it. Run: node scripts/render-model-lp.mjs [out.png] [azimuthDeg] [dist] [ty] [tx] [ex] [ey]
...
const OUT = process.argv[2] || "/tmp/model-lp.png";
const AZ = Number(process.argv[3] ?? 20);
const DIST = Number(process.argv[4] ?? 3.7);
const TY = Number(process.argv[5] ?? 1.05);
const TX = Number(process.argv[6] ?? 0);
...
const scene = new THREE.Scene(); scene.background = new THREE.Color("#0e151f");
...
const az = Number(q.get("az") || "20") * Math.PI / 180, dist = Number(q.get("dist") || "3.7");
const ty = Number(q.get("ty") || "1.05"), tx = Number(q.get("tx") || "0");
const camera = new THREE.PerspectiveCamera(36, W / H, 0.1, 100);
camera.position.set(tx + Math.sin(az) * dist, ty + 0.3, Math.cos(az) * dist); camera.lookAt(tx, ty, 0);
const ex = Number(q.get("ex") || "0"), ey = Number(q.get("ey") || "0");
const loader = new GLTFLoader();
loader.load("/apps/web/public/world-hosts/model-lp.glb", (gltf) => {
  if (ex || ey) gltf.scene.traverse((o) => { if (o.name && o.name.startsWith("eyeIris")) { o.position.x += ex; o.position.y += ey; } });
  scene.add(gltf.scene); renderer.render(scene, camera); window.__done = true;
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (pathname === "/" || pathname === "/index.html") { res.writeHead(200, { "content-type": "text/html" }); res.end(PAGE); return; }
...
await page.goto(`http://localhost:8097/?az=${AZ}&dist=${DIST}&ty=${TY}&tx=${TX}&ex=${EX}&ey=${EY}`, { waitUntil: "load" });
