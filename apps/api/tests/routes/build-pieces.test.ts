...
describe("build pieces routes", () => {
  ...
  it("places wall-corner pieces via batch with their corner slot", async () => {
    const app = await buildTestApp({ config: buildPiecesConfig(), repository: new MemoryRepository() });
    const { classRecord, roomWithManifest } = await createFfaRoom(app);
    const roomId = roomWithManifest.room.id;
    await addStudentMember(app, classRecord.id, "teacher-ffa", "builder-a", "Alex");
    ...
    const batchRes = await app.inject({
      ...
      payload: {
        pieces: [
          { kind: "wall-corner", cell: { ix: 15, iz: 15 }, level: 0, corner: "ne", materialId: "stone" },
          { kind: "wall-corner", cell: { ix: 15, iz: 15 }, level: 0, corner: "nw", materialId: "wood" }
        ]
      }
    });
    expect(batchRes.statusCode).toBe(200);
    expect(batchRes.json().pieces).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ kind: "wall-corner", corner: "ne", id: "build:wall-corner:15,15:0:ne" }),
        expect.objectContaining({ kind: "wall-corner", corner: "nw", id: "build:wall-corner:15,15:0:nw" })
      ])
    );
  ...
  it("rejects wall-corner batch pieces missing corner", async () => {
    const app = await buildTestApp({ config: buildPiecesConfig(), repository: new MemoryRepository() });
    const { classRecord, roomWithManifest } = await createFfaRoom(app);
    const roomId = roomWithManifest.room.id;
    await addStudentMember(app, classRecord.id, "teacher-ffa", "builder-a", "Alex");
    ...
    const batchRes = await app.inject({
      ...
      headers: authHeaders("builder-a", "Alex"),
      payload: {
        pieces: [{ kind: "wall-corner", cell: { ix: 15, iz: 15 }, level: 0, materialId: "stone" }]
      }
    });
    expect(batchRes.statusCode).toBe(400);
    expect(batchRes.json().error).toBe("validation_error");
