...
// The committed MODEL-LP host model (scripts/build-model-lp-glb.mjs) is the
// default World Host avatar and doubles as a placeable room object. This guards
// that it always satisfies the same gate a user upload goes through, so it can be
// imported as an object.
const GLB_PATH = resolve(process.cwd(), "apps/web/public/world-hosts/model-lp.glb");
...
describe("MODEL-LP GLB import compatibility", () => {
  it("passes the custom room-object upload validator", async () => {
    ...
    // Detailed but comfortably inside the triangle + file-size budgets.
