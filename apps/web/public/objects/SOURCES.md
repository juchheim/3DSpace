...
- **Format:** glTF 2.0 binary, ~3.6 MB. Native footprint ~7.6 × 8.4 × 9.2 m after neck crop; scaled to
  160% at placement (`scale: 1.6` → ~12 × 13 × 15 m). Floor at local y=0 (lowest stair tread).
  Grayscale baked texture. Walkable via `staticCollider: true` (Rapier trimesh).
