...
export function ClassroomPanel({
  ...
  async function submitPrivateCheck(check: ClassroomPrivateCheck) {
    ...
    if (submitted) {
      setStudentAnswers((current) => {
        const { [check.id]: _submittedAnswer, ...rest } = current;
        return rest;
      });
    }
  }
  ...
  if (role === "teacher") {
    return (
      <div className="hud-card" aria-label="Classroom tools">
        ...
        <div className="classroom-section-divider" />
        <div className="hud-heading">
          <span>Private Checks</span>
          <span>{privateChecks.filter((check) => check.status === "open").length} open</span>
        </div>
  ...
  return (
    <div className="hud-card" aria-label="Classroom tools">
      ...
      <div className="hud-heading">
        <span>Private Checks</span>
        <span>{loading ? "…" : activeStudentChecks.length}</span>
      </div>
